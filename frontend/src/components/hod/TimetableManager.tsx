...
import { Calendar, Trash, Plus, X, BookOpen, User, MagicWand, CheckCircle, DoorOpen } from '@phosphor-icons/react';
...
interface TimetableConfig {
  ...
  working_days: string[];
  breaks: { type: string; after_period: number; duration_mins: number }[];
}
...
export default function TimetableManager({ user }: Props) {
  // Context
  ...
  const [settings, setSettings] = useState<TimetableConfig | null>(null);
  ...
  const [generatedPreview, setGeneratedPreview] = useState<TimetableSlot[] | null>(null);
  ...
  // Subject colour map — stable per render
  const subjectColorMap = React.useRef<Record<string, string>>({});
  let colorIdx = 0;
  const getSubjectColor = (code: string): string => {
    if (!subjectColorMap.current[code]) {
      subjectColorMap.current[code] = SLOT_COLORS[colorIdx % SLOT_COLORS.length];
      colorIdx++;
    }
    return subjectColorMap.current[code];
  };
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const dept: DeptInfo = deptRes.data;
        setDeptInfo(dept);
        setTeachers(teacherRes.data ?? []);

        const cfg = settingsRes.data?.settings?.timetable_config;
        setSettings(cfg ?? {
          periods_per_day: 8,
        ...
        console.error('TimetableManager bootstrap error:', e);
      }
    };
    load();
  }, []);

  // Load subjects filtered by semester
  useEffect(() => {
    if (!deptInfo) return;
    const loadSubjects = async () => {
      try {
        const res = await api.get('/hod/timetable/subjects', { params: { semester } });
  ...
  // Derive user batch as default
  useEffect(() => {
    const userBatch = (user as Record<string, string>)?.batch || '';
    if (userBatch && !batch) setBatch(userBatch);
  }, [user]);

  // --- Fetch timetable ---
  const fetchTimetable = useCallback(async () => {
    if (!deptInfo || !batch) return;
    setLoading(true);
    try {
      const res = await timetableAPI.getHod(deptInfo.id, batch, section, academicYear);
      setSlots(res.data ?? []);
    } catch (err) {
      console.error('Failed to fetch timetable:', err);
    }
    setLoading(false);
  }, [deptInfo, batch, section, academicYear]);
  ...
  const getSlot = (day: string, period: number) =>
    slots.find(s => s.day === day && s.period_no === period);

  // --- Allocation wizard ---
  const addAllocationRow = () => {
    setAllocations([...allocations, {
      subject_code: '', subject_name: '', faculty_id: '',
      hours_per_week: 4, is_lab: false, room: ''
    }]);
  };

  const updateAllocation = (index: number, field: keyof Allocation, value: string | number | boolean) => {
    const next = [...allocations];
    next[index] = { ...next[index], [field]: value };

    if (field === 'subject_code') {
      const subj = subjects.find(s => s.code === value);
      if (subj) {
        next[index].subject_name = subj.name;
        next[index].is_lab = subj.is_lab;
        next[index].hours_per_week = subj.hours_per_week ?? 4;
      }
    }
    setAllocations(next);
  };
  ...
  const handleGenerate = async () => {
    if (!deptInfo || !batch) {
      showAlert('Missing Info', 'Select a batch before generating.', 'warning');
      return;
    }
    setGenerating(true);
    setErrorMsg(null);
    try {
      ...
      setErrorMsg(detail || 'Generation failed. Check allocations and try again.');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const commitTimetable = async () => {
    ...
    try {
      ...
      showAlert('Committed', 'Timetable saved successfully.', 'success');
      setGeneratedPreview(null);
      setShowWizard(false);
      fetchTimetable();
    } catch {
      showAlert('Error', 'Failed to save timetable. Please try again.', 'danger');
    }
  };

  // Batch options — typically 4 years back from now
  const currentYear = new Date().getFullYear();
  const batchOptions = [`${currentYear - 3}-${currentYear + 1}`, `${currentYear - 2}-${currentYear + 2}`, `${currentYear - 1}-${currentYear + 3}`, `${currentYear}-${currentYear + 4}`];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h4 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            Timetable Manager
            {deptInfo && <span className="text-sm font-semibold text-indigo-400 ml-1">— {deptInfo.name}</span>}
          </h4>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <select
            ...
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-slate-200"
          >
            {['A', 'B', 'C', 'D', '1', '2', '3'].map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          {/* Semester */}
          <select
            value={semester}
          ...
          <button
            onClick={() => {
              setAllocations([{ subject_code: '', subject_name: '', faculty_id: '', hours_per_week: 4, is_lab: false, room: '' }]);
              setGeneratedPreview(null);
              setShowWizard(true);
            }}
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#1A202C] shadow-sm">
        {loading || !settings ? (
          <div className="p-12 text-center text-slate-500">
            {!batch ? 'Select a batch to view the timetable.' : 'Loading timetable grid...'}
          </div>
        ) : (
      ...
      {generatedPreview && (
        <div className="flex items-center justify-end gap-3 p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl border border-indigo-100 dark:border-indigo-500/20">
          ...
          <button
            onClick={() => { setGeneratedPreview(null); fetchTimetable(); }}
          ...
          <button
            onClick={commitTimetable}
            className="flex items-center gap-2 px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-lg shadow-md shadow-indigo-500/20 transition-all"
          >
        ...
        {showWizard && !generatedPreview && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-5xl p-6 sm:p-8 border border-slate-100 dark:border-slate-800"
            ...
            >
              <div className="flex items-center justify-between mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Map subjects to faculty — the engine solves constraints automatically.
                    {subjects.length === 0 && (
                      <span className="text-amber-500 ml-2">⚠ No subjects found for Sem {semester}. Create faculty assignments first.</span>
                    )}
                  </p>
                </div>
                  ...
                  <div key={idx} className="grid grid-cols-12 gap-3 items-center bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700/50">
                    ...
                    <div className="col-span-2">
                      <input
                        ...
                        value={alloc.room}
                        onChange={(e) => updateAllocation(idx, 'room', e.target.value)}
                        className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                      />
                    </div>
              ...
              <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                <button
                  ...
                  disabled={generating || allocations.length === 0 || !batch}
