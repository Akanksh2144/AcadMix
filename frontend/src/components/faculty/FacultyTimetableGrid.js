...
/**
 * FacultyTimetableGrid — shared component used by Faculty Dashboard
 * @param {string} mode - 'view' (read-only grid), 'plan' (teaching plan editor), 'record' (class record editor)
 */
const FacultyTimetableGrid = ({ mode = 'view' }) => {
  const [slots, setSlots] = useState([]);
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedSlot, setSelectedSlot] = useState(null); // for plan/record modal
  const [formData, setFormData] = useState({ planned_topic: '', actual_topic: '', methodology: '', remarks: '' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');

  // Stabilize "today" so it doesn't change on every render
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [slotsRes, recordsRes] = await Promise.all([
        timetableAPI.getFacultyWeek(),
        mode !== 'view' ? facultyPanelAPI.teachingRecords({ month: today.getMonth() + 1, year: today.getFullYear() }) : Promise.resolve({ data: [] }),
      ]);
  ...
  // Get dates for each day in this week
  ...
  const handleSave = async () => {
    ...
    try {
      if (mode === 'plan') {
        ...
        if (selectedSlot.record?.id) {
          await facultyPanelAPI.updateTeachingRecord(selectedSlot.record.id, { planned_topic: formData.planned_topic });
        } else {
          await facultyPanelAPI.saveTeachingPlan({ period_slot_id: selectedSlot.slot.id, date: selectedSlot.date, planned_topic: formData.planned_topic });
        }
        ...
        if (selectedSlot.record?.id && selectedSlot.record?.is_class_record_submitted) {
          await facultyPanelAPI.updateTeachingRecord(selectedSlot.record.id, { actual_topic: formData.actual_topic, methodology: formData.methodology, remarks: formData.remarks });
        } else {
          await facultyPanelAPI.saveClassRecord({ period_slot_id: selectedSlot.slot.id, date: selectedSlot.date, actual_topic: formData.actual_topic, methodology: formData.methodology, remarks: formData.remarks });
        }
