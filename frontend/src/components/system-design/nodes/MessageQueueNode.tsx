...
function MessageQueueNode({ id, data }: NodeProps & { data: any }) {
  return (
    <div
      ...
      style={{ borderLeftColor: '#D08C4A', fontFamily: "'Caveat', cursive" }}
    >
      <div className="px-4 pt-3 pb-1 flex items-center gap-2">
        <Queue size={16} weight="bold" style={{ color: '#D08C4A' }} />
        <span className="text-base font-bold text-[#2D2D2D] dark:text-[#D4D4D4] font-[Caveat]">Message Queue / Stream</span>
      </div>
      ...
      {data.metrics && (
        <div className="px-3 py-2 border-t border-[#E0D9CB] dark:border-[#2E3545] bg-[#F5F0E8]/60 dark:bg-[#161B28]/60 text-sm font-[Caveat]">
          <div className="flex items-center justify-between">
            <span className="text-[#6B6B6B] dark:text-[#8B8B9A] font-semibold">Backlog: {data.metrics.droppedQPS > 0 ? 'Lagging' : 'Healthy'}</span>
            <span className="font-bold text-[#D08C4A]">{data.metrics.processedQPS.toFixed(0)}/s</span>
          </div>
        </div>
      )}
