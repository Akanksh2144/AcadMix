/**
 * System Design Arena — Component Registry
 *
 * Centralized registry of all 40 system design components across 9 architectural categories.
 * Provides metadata, icons, styling colors, and default simulation parameters.
 */
...
export const PALETTE_SECTIONS: PaletteSection[] = [
  {
    ...
    items: [
      ...
      { type: 'dns', label: 'DNS', category: 'Edge & Routing', icon: Globe, color: 'var(--accent-blue)', defaults: { label: 'DNS / Route53', ttl: 300, routingPolicy: 'round-robin' } },
  ...
  },
  {
    ...
    items: [
      { type: 'appServer', label: 'App Server', category: 'Compute & Orchestration', icon: Desktop, color: 'var(--accent-blue)', defaults: { label: 'App Server / API', replicas: 1, maxThreads: 200, processingTime: 50 } },
      ...
      { type: 'serverless', label: 'Serverless Function', category: 'Compute & Orchestration', icon: Cloud, color: '#F59E0B', defaults: { label: 'AWS Lambda / Function', coldStartMs: 150, memoryMb: 512, processingTime: 30 } },
  ...
  },
  {
    ...
    items: [
      ...
      { type: 'memcached', label: 'Memcached', category: 'Caching & In-Memory', icon: Cpu, color: '#EF4444', defaults: { label: 'Memcached Pool', hitRatio: 0.85, threadsPerNode: 8, maxSize: 512 } },
  ...
  },
  {
    ...
    items: [
      ...
      { type: 'rateLimiter', label: 'Rate Limiter', category: 'Security & Identity', icon: Gauge, color: '#F97316', defaults: { label: 'Token Bucket Limiter', maxQpsPerUser: 100, algorithm: 'token-bucket' } },
    ],
  },
  {
