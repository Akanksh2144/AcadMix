...
async def generate_problem(topic: str, difficulty: str) -> dict:
    ...
    Self-Correction: Re-verify the full length of the array or string bounds manually against your own problem constraints before finalizing the expected output. Double-check your monotonic stack bounds and prefix sum logic to prevent math bugs. CRITICAL: ALL self-correction and re-checking MUST happen ONLY in the 'step_by_step_trace' field. The 'description' field (including Examples, Explanation sections) must NEVER contain phrases like "Wait", "Actually", "Hmm", "Let me recheck", "Correct trace:", "← Wait", or any chain-of-thought reasoning. Each example block must have EXACTLY ONE Input:, ONE Output:, and ONE Explanation: — no duplicates, no re-tracing.
