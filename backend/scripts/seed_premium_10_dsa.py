...
async def generate_problem(topic: str, difficulty: str) -> dict:
    ...
    Real-World Enterprise Reskinning: Frame the problem narrative around highly diversified, modern tech-industry engineering contexts. DO NOT restrict themes to academic, campus, or hostel life. Ensure massive diversity across B2B SaaS, FinTech, DeepTech, and E-commerce.
    No Solution Leaking: DO NOT leak the name of the algorithmic pattern (e.g., "This is a classic Greedy problem" or "Using Dynamic Programming") anywhere in the `description`. The problem statement must only explain the business rules. Explaining the solution goes in `coach_instructions`.
    Formal Constraints: You MUST strictly append a `### Constraints` section at the very end of your `description` containing bulleted strict mathematical guarantees (e.g., `- 1 <= N <= 10^5`). Specify exact variable limits.
    ...
    Example Restraint & Quality: In the `description`, examples MUST be minimal, authoritative, and clean. Write a concise explanation (2-3 logic steps). NEVER enumerate 10 brute-force options. NEVER output any "think aloud" or "wait, maybe" chain-of-thought in the `description`.
    ...
    Problem AI Context Paragraphs: In `real_world_applications`, you MUST use `\n\n` to strictly separate your ideas into distinct paragraphs. Use bolding `**1. Title:**` for your list items. Never output separated asterisks.
    ...
    Inputs & Memory Objects: 'input_data' will be evaluated dynamically by Python via `eval()`. If the problem involves Linked Lists or Trees, the testing sandbox natively provides `ListNode` and `TreeNode` classes, alongside `build_linked_list(arr)` and `build_tree(arr)` parsers. FORMAT YOUR `input_data` STRING TO EXPLICITLY CALL THESE HELPERS. Example: `input_data: "(build_tree([1, 2, 3, null, 4]), 5)"`.
    Test Case Accuracy: Use the 'step_by_step_trace' field to manually execute your algorithm on the 'input_data'. For hidden test cases, keep the step_by_step_trace extremely brief (1 line) to conserve API tokens!
    Self-Correction Strict Bans: ALL self-correction and re-checking MUST happen ONLY in the 'step_by_step_trace' field. The 'description' field (including Examples, Explanation sections) must NEVER contain phrases like "Wait", "Actually", "Hmm", "Let me recheck", "Correct trace:", "← Wait", or any chain-of-thought reasoning. Each example block must have EXACTLY ONE Input:, ONE Output:, and ONE Explanation: — no duplicates, no re-tracing.
