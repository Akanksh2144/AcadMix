"""
Interview War Room — AI Mock Interview Router
Handles interview lifecycle: quota checking, session start/message/end,
history retrieval, and interview readiness scoring.
Uses SSE (StreamingResponse) for real-time AI response streaming.
"""
...
logger = logging.getLogger("acadmix.interview")
...
async def _call_llm(messages: list, model: str = None, json_mode: bool = False) -> str:
...
async def _stream_llm(messages: list, model: str = None):
...
async def start_interview(
    ...
    Body: { interview_type, target_role, target_company?, difficulty? }
    """
    # Check quota
    now = datetime.now(timezone.utc)
    stmt = (
    ...
    first_question = await _call_llm(messages)
...
async def send_message(
    ...
    Uses SSE streaming for real-time token delivery.
    Body: { content: "student's spoken/typed answer" }
    """
    ...
    student_content = req.get("content", "").strip()
    ...
    ai_response = await _call_llm(llm_messages)
